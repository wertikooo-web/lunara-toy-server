...
function isStoryRequest(text) {
    ...
    if (/(что ты|что это|классно|нравится|понравилось|не надо|не хочу|хватит|стоп)/i.test(value)) {
        return false;
    }
...
async function buildStoryContext(userText) {
    ...
    return {
        ...
        prompt: [
            ...
            '',
            'Ответь как Lumi. Сразу расскажи историю, без вступления. Заверши историю полностью.',
        ].join('\n'),
        maxTokens: 230,
    };
}
