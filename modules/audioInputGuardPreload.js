...
function isRussianLang(lang) {
    return !lang || String(lang).toLowerCase().startsWith('ru');
}

function isSuspiciousTranscript(text, lang) {
    const raw = String(text || '').trim();
    const t = normalizeText(raw);
    if (!t) return false;

    // Common Whisper hallucinations when input is too short, noisy or silent.
    // These fire regardless of device language — genuine hallucination signatures.
    ...
    if (!hasCyrillic(raw)) {
        ...
        // Only for Russian-language devices: a short pure-Latin phrase is likely a
        // foreign hallucination. For devices actually configured in en/ro/es/fr/it,
        // short native replies ("Da", "Salut", "Bine", "Hi") are completely normal
...
function isSuspiciousForeignTranscript(text, lang) {
    return isSuspiciousTranscript(text, lang);
}
...
function patchServerSource(source) {
    ...
    patched = replaceOnce(
        ...
        "        logger.info(`[Pipeline] saved input PCM: ${pcmBuffer.length} bytes`);\n\n        if (isTooShortForStt(pcmBuffer)) {\n            logger.info(`[AudioInputGuard] skipped STT: pcm_too_short bytes=${pcmBuffer.length} min=${MIN_STT_PCM_BYTES}`);\n            const pcmGuardSettings = await parentConfig.getSettings(deviceId);\n            const r = await retryAudioCommand(pcmGuardSettings.language, pcmGuardSettings.toyGender || pcmGuardSettings.toy_gender, buildVoiceConfig(pcmGuardSettings), pcmGuardSettings.voice_speed);\n            if (r) sendAudio(r.url, r.durationMs);\n            return;\n        }\n\n        // 3. STT — Whisper",
    ...
    patched = replaceOnce(
        ...
        "        if (!transcript || transcript.trim().length === 0) {\n            logger.info('[Pipeline] empty transcript — Lumi gently asks to repeat');\n            const emptyTranscriptSettings = await parentConfig.getSettings(deviceId);\n            const r = await retryAudioCommand(emptyTranscriptSettings.language, emptyTranscriptSettings.toyGender || emptyTranscriptSettings.toy_gender, buildVoiceConfig(emptyTranscriptSettings), emptyTranscriptSettings.voice_speed);\n            if (r) sendAudio(r.url, r.durationMs);\n            return; // finally{} сбросит state в IDLE и удалит upload\n        }\n\n        if (isSuspiciousTranscript(transcript, sttSettings.language)) {\n            logger.info(`[AudioInputGuard] skipped LLM: suspicious_transcript chars=${String(transcript || '').length}`);\n            const suspiciousTranscriptSettings = await parentConfig.getSettings(deviceId);\n            const r = await retryAudioCommand(suspiciousTranscriptSettings.language, suspiciousTranscriptSettings.toyGender || suspiciousTranscriptSettings.toy_gender, buildVoiceConfig(suspiciousTranscriptSettings), suspiciousTranscriptSettings.voice_speed);\n            if (r) sendAudio(r.url, r.durationMs);\n            return;\n        }\n\n        // 4. LLM — Claude",
